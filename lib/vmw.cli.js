...
// main
async function main(category, version, type, downloadGroup) {
	client.main(category, version, type, downloadGroup).then((data) => {
...
async function cmdPath(string,downloadGroup) {
	...
	if(path.length == 1) {
		...
		if(path[1].length > 0) {
			if(downloadGroup !== undefined){
				main(path[0], path[1],undefined,downloadGroup);
			}else {
				main(path[0],path[1]);
			}
